...
class Tieefseeview {
    constructor(_dom) {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (_scale != temp_drawImage.scale
                    ...
                    if (_scale >= 1) {
                        ...
                        yield createImageBitmap(temp_can, 0, 0, sWidth, sHeight, { resizeWidth: dWidth, resizeHeight: dHeight, resizeQuality: resizeQuality })
                        ...
                        //dWidth = getOriginalWidth() * _scale
                        dHeight = getOriginalHeight() * _scale;
                        yield createImageBitmap(temp_can, sx, 0, sWidth, sHeight, { resizeWidth: dWidth, resizeHeight: dHeight, resizeQuality: resizeQuality })
                            .then(function (sprites) {
                            if (tc !== temp_canvasSN) {
                                return;
                            }
                        ...
                        dWidth = getOriginalWidth() * _scale;
                        //dHeight = getOriginalHeight() * _scale
                        yield createImageBitmap(temp_can, 0, sy, sWidth, sHeight, { resizeWidth: dWidth, resizeHeight: dHeight, resizeQuality: resizeQuality })
                            .then(function (sprites) {
                            if (tc !== temp_canvasSN) {
                                return;
                            }
        ...
        function init_scroll() {
            scrollX.init_size(dom_con.offsetWidth + marginLeft + marginRight, dom_dpizoom.offsetWidth, toInt(dom_con.style.left) * -1 + marginLeft);
            scrollY.init_size(dom_con.offsetHeight + marginTop + marginBottom, dom_dpizoom.offsetHeight, toInt(dom_con.style.top) * -1 + marginTop);
        }
        /**
        ...
        function setMirrorHorizontal(bool, boolAnimation = true) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                //取得中心點在旋轉前的實際坐標
                let origPoint = getOrigPoint(left, top, toInt(dom_data.style.width), toInt(dom_data.style.height), degNow);
                left = origPoint.x;
                top = origPoint.y;
                /*//計算鏡像後的坐標
                ...
                //取得旋轉回原本角度的坐標
                let rotateRect = getRotateRect(toInt(dom_data.style.width), toInt(dom_data.style.height), left, top, degNow);
                left = rotateRect.x;
                top = rotateRect.y;
                //轉換成定位用的值，並移動回中心點
