...
class Msgbox {
    ...
    public static show(json: {
        ...
        let funcYes = (dom: HTMLElement, inputTxt: string) => { this.close(dom); }

        if (json.txt !== undefined) { txt = json.txt }
        if (json.isInput !== undefined) { isInput = json.isInput }
        ...
        if (isInput === false) { donInput.style.display = "none"; }//沒有輸入框
